...
checkpoint_starch = tf.keras.callbacks.ModelCheckpoint(
                    filepath=f"{save_file_path}{Code_run_ID}_model_file_starch_hybrid.keras", 
...
with strategy.scope():
    ...
    model_hybrid_starch = create_hybrid_model(
    ...
    # Compile the model
    model_hybrid_starch.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=0.0005),
        loss="mean_squared_error", 
        metrics=["mae"]
    )

model_hybrid_starch.summary()
...
csv_logger = CSVLogger(f"{save_file_path}{Code_run_ID}_history_starch_hybrid.csv", append=True)
...
weight_decay_callback = tf.keras.callbacks.LambdaCallback(
    on_epoch_end=lambda epoch, logs: [
        w.assign(w * (1 - 1e-5)) 
        for w in model_hybrid_starch.trainable_weights 
        if 'kernel' in w.name
    ]
)

# 训练模型
history = model_hybrid_starch.fit(
    train_generator,
    epochs=100,
    ...
    callbacks=[
        tf.keras.callbacks.CSVLogger(f"{save_file_path}{Code_run_ID}_history_starch_hybrid.csv", append=True), 
        checkpoint_starch,
        early_stopping,
        reduce_lr,
...
model_hybrid_starch.save(f'{save_file_path}{Code_run_ID}_model_trained_hybrid_starch.keras')


# In[ ]:


del model_hybrid_starch
gc.collect()

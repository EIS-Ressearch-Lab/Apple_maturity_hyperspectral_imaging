...
with strategy.scope():
    ...
    model_complex_firmness.add(MaxPooling2D(pool_size=(2, 2), strides=(1,1), padding = 'same')) # cuts image in half
    ...
    model_complex_firmness.add(Dropout(0.21))
...
csv_logger = CSVLogger(f"{save_file_path}{Code_run_ID}_{today}_history_firmness.csv", append=True)
...
model_complex_firmness.fit(train_generator,epochs=110,steps_per_epoch=len(X_train_Firmness) // batch_size,validation_data=val_generator,validation_steps=len(X_validate_Firmness) // batch_size,callbacks=[tf.keras.callbacks.CSVLogger(f"{save_file_path}{Code_run_ID}_{today}history_firmness.csv", append=True), checkpoint_firmness],)


# In[ ]:


model_complex_firmness.save(f'{save_file_path}{Code_run_ID}_{today}model_trained_2DCNN_firmness.keras')

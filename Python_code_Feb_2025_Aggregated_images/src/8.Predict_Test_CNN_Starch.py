...
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_trained_2DCNN_starch.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_trained_2DCNN_starch.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*history_starch.csv'
print(pattern)
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    plt.xlabel('Actual Starch Value')
    plt.ylabel('Predicted Starch Value')
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_error_distribution.png', dpi=300)
    plt.show()
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/starch_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.title('Actual vs Predicted by Cultivar')
    plt.xlabel('Actual Starch Value')
    plt.ylabel('Predicted Starch Value')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_cultivar_predictions.png', dpi=300)
    plt.show()

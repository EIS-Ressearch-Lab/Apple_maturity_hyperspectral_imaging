...
def find_latest_model(model_dir=results_path):
    model_files = glob.glob(f"{model_dir}/*model_file_firmness_hybrid.keras")
    if not model_files:
...
# model_path = f'{results_path}{run_no}_2025-03-10model_trained_hybrid_firmness.keras'  # 模型文件路径
# history_path = f'{results_path}{run_no}_2025-03-10history_firmness_hybrid.csv'  # 训练历史文件路径 # 
...
pattern = f'{results_path}/*{run_no}*_history_firmness_hybrid.csv'
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_hybrid_training_history.png', dpi=300)
    plt.show()
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_hybrid_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_hybrid_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/firmness_hybrid_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/firmness_hybrid_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_hybrid_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_hybrid_cultivar_predictions.png', dpi=300)
    plt.show()

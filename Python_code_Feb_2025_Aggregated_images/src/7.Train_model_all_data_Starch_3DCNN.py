...
def create_3d_cnn_model(spectral_input_shape, cultivar_input_shape, filters=(32, 64, 128, 160, 192), dropout_rate=0.3):
    ...
    # 输出层 - 回归任务
    output = Dense(1, name='starch_output')(x)
...
checkpoint_starch = tf.keras.callbacks.ModelCheckpoint(
                    filepath=f"{save_file_path}{Code_run_ID}_model_file_starch_3dcnn.keras", 
...
with strategy.scope():
    ...
    # 4. 增加全连接层的神经元数量
    # 5. 减小批次大小以适应更复杂的模型
    model_3dcnn_starch = create_3d_cnn_model(
...
csv_logger = CSVLogger(f"{save_file_path}{Code_run_ID}_history_starch_3dcnn.csv", append=True)
...
weight_decay_callback = tf.keras.callbacks.LambdaCallback(
    on_epoch_end=lambda epoch, logs: [
        w.assign(w * (1 - 1e-5)) 
        for w in model_3dcnn_starch.trainable_weights 
        if 'kernel' in w.name
    ]
)

# 使用数据增强的训练生成器，验证集不使用增强
train_generator = data_generator_3d(X_train_Starch, Y_train_Starch, Starch_encoder_shuffled, batch_size, img_size=img_size, augment=True)
val_generator = data_generator_3d(X_validate_Starch, Y_validate_Starch, validate_encoder, batch_size, img_size=img_size, augment=False)
...
history = model_3dcnn_starch.fit(
    train_generator,
    epochs=epochs,
    ...
    callbacks=[
        tf.keras.callbacks.CSVLogger(f"{save_file_path}{Code_run_ID}_history_starch_3dcnn.csv", append=True), 
        checkpoint_starch,
        early_stopping,
        reduce_lr,
...
# 保存最终模型
model_3dcnn_starch.save(f'{save_file_path}{Code_run_ID}_model_trained_3dcnn_starch.keras')

...
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results'):
    model_files = glob.glob(f"{model_dir}/*model_file_firmness_3dcnn.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*_history_firmness_3dcnn.csv'
...
if not model_path:
    ...
    model_path = '/media/2tbdisk3/data/Haidee/Results/2025-05-27run_9DCNN_50px_model_file_firmness_3dcnn.keras'
...
try:
    ...
    print(f"X_test_firmness: {X_test_firmness.shape}, {type(X_test_firmness[0])}")
    print(f"Y_test_firmness: {Y_test_firmness.shape}")
...
# 检查第一个文件是否可访问
if len(X_test_firmness) > 0:
    first_file = X_test_firmness[0]
...
for [spectral_data, cultivar_data], valid_indices in data_generator_3d_predict(X_test_firmness, test_encoder, batch_size=batch_size, img_size=img_size):
    total_batches += 1
    try:
        ...
        batch_actual = Y_test_firmness[valid_indices]
        
        predictions.extend(batch_predictions)
        actual_values.extend(batch_actual)
        
        # 打印进度 # Print progress
        print(f"Processed; 已处理 {len(predictions)}/{len(X_test_firmness)} samples; 个样本")
    except Exception as e:
        failed_batches += 1
...
# })
# results_df.to_csv(f'{results_dir}/{run_no}_firmness_prediction_results.csv', index=False)
# print(f"Results have been saved to; 结果已保存到 {results_dir}/firmness_prediction_results.csv")
...
# plt.tight_layout()
# plt.savefig(f'{results_dir}/{run_no}_firmness_prediction_visualization.png', dpi=300, bbox_inches='tight')
# print(f"Graphs saved to; 可视化图表已保存到 {results_dir}/{run_no}_firmness_prediction_visualization.png")
...
# })
# metrics_df.to_csv(f'{results_dir}/{run_no}_firmness_evaluation_metrics.csv', index=False)
# print(f"Evaluation metrics have been saved to; 评估指标已保存到 {results_dir}/{run_no}_firmness_evaluation_metrics.csv")
...
def visualize(predictions, actual_values, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_3dCNN_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_3dCNN_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/firmness_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/firmness_3dCNN_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_3dCNN_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_3dCNN_cultivar_predictions.png', dpi=300)
    plt.show()
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_3dCNN_training_history.png', dpi=300)
    plt.show()
...
# 主函数 # Main function
def main():
    print("Start visualistion of hyfirmness model results; 开始混合模型结果可视化...") 

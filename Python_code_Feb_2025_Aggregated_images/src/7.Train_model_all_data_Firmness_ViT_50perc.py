...
img_size = 40
band_filter = 50
Code_run_ID = today + f'run_22_top{band_filter}perc_{img_size}px'
...
if band_filter is not None:
    channel_size = np.sum(selected_features_threshold_firmness) + 6
...
num_attention_layers = min_firmness_dict['transformer_layers']  # Number of attention layers to monitor
...
# X_train_Firmness          = np.load(f'{training_data_path}X_train_NZ2023_Firmness_shuffled.npy')
# Y_train_Firmness          = np.load(f'{training_data_path}Y_train_NZ2023_Firmness_shuffled.npy')
# X_validate_Firmness       = np.load(f'{training_data_path}X_validate_NZ2023_Firmness_shuffled.npy')
# Y_validate_Firmness       = np.load(f'{training_data_path}Y_validate_NZ2023_Firmness_shuffled.npy')
# Frmness_encoder_shuffled  = np.load(f'{training_data_path}X_train_NZ2023_Firmness_encoder_shuffled.npy')
# validate_encoder        = np.load(f'{training_data_path}X_validate_NZ2023_Firmness_encoder_shuffled.npy')
...
# 使用数据增强的训练生成器
train_generator = data_generator_w_cultivar(X_train_Firmness, Y_train_Firmness, Frmness_encoder_shuffled, batch_size, img_size=img_size, band_filter=selected_features_threshold_firmness)
val_generator = data_generator_w_cultivar(X_validate_Firmness, Y_validate_Firmness, validate_encoder, batch_size, img_size=img_size, band_filter=selected_features_threshold_firmness)
...
plt.tight_layout()
plt.savefig(f"{save_file_path}{Code_run_ID}_training_history_firmness.png", dpi=300)
plt.show()

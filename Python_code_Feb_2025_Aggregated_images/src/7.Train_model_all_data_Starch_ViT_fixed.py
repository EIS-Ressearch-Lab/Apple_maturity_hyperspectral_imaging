...
img_size = 40
Code_run_ID = today + f'run_20_{img_size}px'
...
def create_vit_model(
    ...
    # Create the model
    model = Model(inputs=inputs, outputs=[outputs] + attention_outputs)
    return model
...
#     model_vit_starch.compile(
    num_attention_layers = min_starch_dict['transformer_layers']
    model_vit_starch.compile(
        optimizer=tf.keras.optimizers.Adam(learning_rate=0.0005),
        loss=["mean_squared_error"] + [None] * num_attention_layers,
        metrics=["mae"] + [None] * num_attention_layers
    )

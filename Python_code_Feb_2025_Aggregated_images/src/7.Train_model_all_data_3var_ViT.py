...
img_size = 40
band_filter = None
Code_run_ID = today + f'run_28_3var_{img_size}px'
channels = 210  # Number of spectral bands
...
if band_filter is not None:
    channel_size = np.sum(selected_features_threshold_var3) + 6
...
def create_vit_model(
    ...
    regression_output = Dense(3, activation="linear", name="regression")(features)
...
checkpoint_var3 = tf.keras.callbacks.ModelCheckpoint(
                    filepath=f"{save_file_path}{Code_run_ID}_model_file_var3_vit.keras", 
...
with strategy.scope():
    ...
    model_vit_var3 = create_vit_model(
    ...
    # Compile the model
    model_vit_var3.compile(optimizer="adam", loss="mean_squared_error", metrics=["mae"])

model_vit_var3.summary()
...
Frmness_encoder_shuffled  = np.load(f'{training_data_path}X_train_all_years_var3_encoder_shuffled.npy')
...
# X_train_var3          = np.load(f'{training_data_path}Side_A_X_train_all_years_var3_shuffled.npy')
# Y_train_var3          = np.load(f'{training_data_path}Side_A_Y_train_all_years_var3_shuffled.npy')
# X_validate_var3       = np.load(f'{training_data_path}Side_A_X_validate_all_years_var3_shuffled.npy')
# Y_validate_var3       = np.load(f'{training_data_path}Side_A_Y_validate_all_years_var3_shuffled.npy')
# Frmness_encoder_shuffled  = np.load(f'{training_data_path}Side_A_X_train_all_years_var3_encoder.npy')
# validate_encoder        = np.load(f'{training_data_path}Side_A_X_validate_all_years_var3_encoder.npy')
...
csv_logger = CSVLogger(f"{save_file_path}{Code_run_ID}_history_var3_vit.csv", append=True)


# In[ ]:


train_generator = fast_data_generator(X_train_var3, Y_train_var3, Frmness_encoder_shuffled, batch_size, img_size=img_size, band_filter=None)
val_generator = fast_data_generator(X_validate_var3, Y_validate_var3, validate_encoder, batch_size, img_size=img_size, band_filter=None)
...
model_vit_var3.fit(
    train_generator,
    epochs=100,
    steps_per_epoch=len(X_train_var3) // batch_size,
    validation_data=val_generator,
    validation_steps=len(X_validate_var3) // batch_size,
    callbacks=[
        tf.keras.callbacks.CSVLogger(f"{save_file_path}{Code_run_ID}_history_var3_vit.csv", append=True), 
        checkpoint_var3,
        early_stopping
    ],
)
...
weight_decay_callback = tf.keras.callbacks.LambdaCallback(
    on_epoch_end=lambda epoch, logs: [
        w.assign(w * (1 - 1e-5)) 
        for w in model_vit_var3.trainable_weights 
        if 'kernel' in w.name
    ]
)
...
model_vit_var3.save(f'{save_file_path}{Code_run_ID}_model_trained_vit_var3.keras')
...
var3_end = datetime.now()
print("总训练时间 Total training time:")
print(var3_end - start_time)
var3_total_time = var3_end - start_time
print(var3_total_time)
print(datetime.now()) 
...
img_size = 40
Code_run_ID = today + f'run_17ViT_NZ2024_{img_size}px'
...
def create_vit_model(
    ...
    num_patches=25,  # Number of patches (depends on image size and patch size)
    projection_dim=64,  # Embedding dimension for each patch
    transformer_layers=3,  # Number of Transformer blocks
    num_heads=4,  # Number of attention heads
    mlp_head_units=[128, 64],  # Hidden units in the MLP head
    ...
    # Create patches
    # First, reshape the input to (batch_size, num_patches, patch_size*patch_size*channels)
...
# Load 2023 data
if img_size == 30 or img_size == 20:
    training_data_path = '/home/ht21074/Auto_box_apples/Auto_box_apples/Source_folder/Training_data_May2025/30px/NZ2024/'
elif img_size == 50 or img_size ==40:
    training_data_path = '/home/ht21074/Auto_box_apples/Auto_box_apples/Source_folder/Training_data_May2025/50px/NZ2024/'


X_train_brix          = np.load(f'{training_data_path}X_train_NZ2024_Brix_shuffled.npy')
Y_train_brix          = np.load(f'{training_data_path}Y_train_NZ2024_Brix_shuffled.npy')
X_validate_brix       = np.load(f'{training_data_path}X_validate_NZ2024_Brix_shuffled.npy')
Y_validate_brix       = np.load(f'{training_data_path}Y_validate_NZ2024_Brix_shuffled.npy')
Frmness_encoder_shuffled  = np.load(f'{training_data_path}X_train_NZ2024_Brix_encoder_shuffled.npy')
validate_encoder        = np.load(f'{training_data_path}X_validate_NZ2024_Brix_encoder_shuffled.npy')

...
def find_latest_model(model_dir=results_path):
    model_files = glob.glob(f"{model_dir}/*model_file_brix_hybrid.keras")
    if not model_files:
...
# model_path = f'{results_path}{run_no}_2025-03-10model_trained_hybrid_brix.keras'  # 模型文件路径
# history_path = f'{results_path}{run_no}_2025-03-10history_brix_hybrid.csv'  # 训练历史文件路径 # 
...
pattern = f'{results_path}/*{run_no}*_history_brix_hybrid.csv'
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_hybrid_training_history.png', dpi=300)
    plt.show()

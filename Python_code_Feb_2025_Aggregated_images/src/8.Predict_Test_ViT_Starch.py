...
selected_features_threshold_starch = None
...
# model_path = f'{results_path}{run_id}_*model_trained_vit_starch.keras'  # 模型文件路径 # Model file path

# 查找最新的模型文件 # Find the latest model files
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_file_starch_vit.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_file_starch_vit.keras")
    if not model_files:
        print(f"Error: No model was found in directory{model_dir}") #print(f"错误: 在 {model_dir} 目录中没有找到模型文件")
        return None
    
    # 按修改时间排序 # Sort by date 
...
pattern = f'{results_path}/*{run_no}*_history_starch_vit.csv'
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    test_generator = data_generator_w_cultivar(X_test, Y_test, test_encoder, batch_size, img_size, band_filter=selected_features_threshold_starch)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_vit_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_vit_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/starch_vit_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/starch_vit_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_vit_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_vit_cultivar_predictions.png', dpi=300)
    plt.show()
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/starch_vit_training_history.png', dpi=300)
    plt.show()

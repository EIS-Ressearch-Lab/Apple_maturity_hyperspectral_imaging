...
selected_features_threshold_firmness = None
...
# model_path = f'{results_path}{run_id}_*model_trained_vit_firmness.keras'  # 模型文件路径 # Model file path

# 查找最新的模型文件 # Find the latest model files
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_file_firmness_vit.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_file_firmness_vit.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*_history_firmness_vit.csv'
...
def data_generator_w_cultivar_swapped(file_list, targets, cultivars, batch_size, img_size, hotencoding = False, band_filter = None):
        ...
        for i, file in enumerate(batch_files):
            try:
                ...
                if img_size == 40 or img_size ==20:
                    ...
                    data_reduced = swap_quadrants(data_reduced)
                    batch_data.append(data_reduced)
                else:
                    swapped_data = swap_quadrants(data)                    
...
def load_test_data(test_data_path):
    ...
    # 加载测试数据
    X_test_firmness = np.load(f'{test_data_path}X_test_all_years_Firmness_shuffled.npy')
    Y_test_firmness = np.load(f'{test_data_path}Y_test_all_years_Firmness_shuffled.npy')
    test_encoder = np.load(f'{test_data_path}X_test_all_years_Firmness_encoder_shuffled.npy')
    
    # 添加路径前缀
    spectral_path = '/media/2tbdisk3/data/Haidee/May_2025_spectral_img/Spectral/'
    X_test_firmness = [spectral_path + file for file in X_test_firmness]
    
    return X_test_firmness, Y_test_firmness, test_encoder
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    test_generator = data_generator_w_cultivar_swapped(X_test, Y_test, test_encoder, batch_size, img_size, band_filter=selected_features_threshold_firmness)
    ...
    plt.xlabel('Actual (Firmness)') #实际糖度
    plt.ylabel('Predicted (Firmness)') # 预测糖度
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_vit_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_vit_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/firmness_vit_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/firmness_vit_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_vit_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_vit_cultivar_predictions.png', dpi=300)
    plt.show()
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_vit_training_history.png', dpi=300)
    plt.show()

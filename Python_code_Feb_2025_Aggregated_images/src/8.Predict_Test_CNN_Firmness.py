...
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_trained_2DCNN_firmness.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_trained_2DCNN_firmness.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*history_firmness.csv'
print(pattern)
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_test_predictions.png', dpi=300)
    plt.show()
    ...
    # 添加误差统计信息
    plt.text(0.05, 0.95, f'Average error: {np.mean(errors):.4f}\nStandard deviation: {np.std(errors):.4f}',
             transform=plt.gca().transAxes, fontsize=12, verticalalignment='top',
             bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))
    
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_error_distribution.png', dpi=300)
    plt.show()
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/firmness_cultivar_metrics.csv', index=False)
    
    # 打印品种评估指标
    print("\Evaluation indicators for various varieties:")
    ...
    plt.title('Average absolute error of each apple variety (MAE)')
    plt.xlabel('Cultivar')
    plt.ylabel('MAE')
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/firmness_cultivar_predictions.png', dpi=300)
    plt.show()
...
# 主函数
def main():
    print("Start visualisation of vit model results...")
    ...
    try:
        ...
        print(f"Test sample len: {len(X_test)}")
    except Exception as e:
        print(f"Error loading test data: {e}")
        return
    
    # 预测和评估
    print("\n3. Predicting and evaluating the test set")
    try:
        results_df, mse, rmse, mae, r2 = predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, results_dir)
    except Exception as e:
        print(f"Error predicting and evaluating: {e}")
        return
    
    # 按品种可视化
    print("\n4. Visualizing prediction performance by cultivar")
    try:
        cultivar_metrics_df = visualize_by_cultivar(results_df, test_encoder, results_dir)
    except Exception as e:
        print(f"Error visualizing by cultivar: {e}")
        return

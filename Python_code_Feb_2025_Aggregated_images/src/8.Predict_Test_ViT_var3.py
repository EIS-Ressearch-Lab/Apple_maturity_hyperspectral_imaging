...
run_no = None
batch_size = 8
band_filter = None #run 21 is 20%, run 22 is 50%, run 23 is 80%
selected_features_threshold_var3 = None
...
if band_filter == 20:
    ...
    selected_features_threshold_var3 = loaded_data['selected_features_threshold_var3_20']
    ...
    selected_features_threshold_var3 = loaded_data['selected_features_threshold_var3_50']
    ...
    selected_features_threshold_var3 = loaded_data['selected_features_threshold_var3_80']
...
img_size = 40
# run_id = None  # 使用之前训练的模型ID
results_path = '/media/2tbdisk3/data/Haidee/Results'

# model_path = f'{results_path}{run_id}_*model_trained_vit_var3.keras'  # 模型文件路径 # Model file path

# 查找最新的模型文件 # Find the latest model files
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_trained_vit_var3.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_trained_vit_var3.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*_history_var3_vit.csv'
print(pattern)
...
# results_dir = f'test_results_{run_id}_{today}'3
results_dir = f'/media/2tbdisk3/data/Haidee/Results/Predictions/{run_no}_ViT_results'
...
# 定义数据生成器函数（与训练文件相同） # Define data generator function (same as training file)
    ...
    while True:  # Infinite loop for generator
        for offset in range(0, num_samples, batch_size):
            ...
            if band_filter is not None:
                ...
                filtered_test = batch_data[:, :, :, band_filter] # % of the features for var3
            ...
            # combined_data = batch_data  # Shape: (batch_size, 50, 50, 204)
...
def load_test_data(test_data_path):
    ...
    # 加载测试数据
    X_test_var3 = np.load(f'{test_data_path}X_test_all_years_var3_shuffled.npy')
    Y_test_var3 = np.load(f'{test_data_path}Y_test_all_years_var3_shuffled.npy')
    test_encoder = np.load(f'{test_data_path}X_test_all_years_var3_encoder_shuffled.npy')
    
    # 添加路径前缀
    spectral_path = '/media/2tbdisk3/data/Haidee/May_2025_spectral_img/Spectral/'
    X_test_var3 = [spectral_path + file for file in X_test_var3]
    
    return X_test_var3, Y_test_var3, test_encoder
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    test_generator = data_generator_w_cultivar(X_test, Y_test, test_encoder, batch_size, img_size, band_filter=selected_features_threshold_var3)
    ...
    plt.title('ViT Models: Actual vs Predicted Values') # 混合模型：实际值 vs 预测值
    plt.xlabel('Actual (var3)') #实际糖度
    plt.ylabel('Predicted (var3)') # 预测糖度
    plt.grid(True)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/var3_vit_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.title('ViT model: Prediction Error Distribution') # 混合模型：预测误差分布 # ViT model: Prediction Error Distribution
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/var3_vit_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/var3_vit_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/var3_vit_cultivar_metrics.csv', index=False)
    ...
    plt.title('ViT model: Mean absolute error for each apple cultivar (MAE)') #混合模型：各苹果品种的平均绝对误差
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/var3_vit_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.title('ViT model: Actual vs Predicted Values by Cultivar') #混合模型：各品种实际值 vs 预测值 # 
    plt.xlabel('Actual (var3)') #实际糖度 # 
    plt.ylabel('Predicted (var3)') #预测糖度
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/var3_vit_cultivar_predictions.png', dpi=300)
    plt.show()
...
def visualize_training_history(history_path, save_dir):
    ...
    # Dynamically find the MAE columns
    if not 'mae' in history_df.columns:
        mae_cols = [col for col in history_df.columns if col.endswith('_mae') and not col.startswith('val')]
        val_mae_cols = [col for col in history_df.columns if col.startswith('val') and col.endswith('_mae')]
        history_df['mae'] = history_df[mae_cols[0]]
        history_df['val_mae'] = history_df[val_mae_cols[0]]
    ...
    plt.title('ViT model loss curve')  #混合模型损失曲线
    ...
    plt.title('ViT model MAE curve') #混合模型MAE曲线
    plt.xlabel('Training rounds') #训练轮次
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/var3_vit_training_history.png', dpi=300)
    plt.show()
...
# 主函数 # Main function
def main():
    print("Start visualistion of vit model results; 开始混合模型结果可视化...") 
    ...
    try:
        ...
        # 如果模型输出是字典，提取“regression”键的输出 # If the model output is a dictionary, extract the output with the "regression" key
        if isinstance(model.output, dict) and "regression" in model.output:
            model = tf.keras.Model(inputs=model.input, outputs=model.output["regression"])
            print("Successfully created model using 'regression' output.")
        model.summary()
    ...
    if img_size == 30 or img_size == 20:
        training_data_path = '/home/ht21074/Auto_box_apples/Auto_box_apples/Source_folder/Training_data_May2025/30px/all_years/all_3_variables/'
    elif img_size == 50 or img_size ==40:
        training_data_path = '/home/ht21074/Auto_box_apples/Auto_box_apples/Source_folder/Training_data_May2025/50px/all_years/all_3_variables/'

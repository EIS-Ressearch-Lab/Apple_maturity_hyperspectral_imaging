...
# ============================================================
# Load early params (for GPU assignment)
# ============================================================
...
# Assign GPU BEFORE importing TF
...
from tensorflow.keras.layers import (
    Conv2D, MaxPooling2D, AveragePooling2D, Dense, BatchNormalization,
    Dropout, GlobalAveragePooling1D, Input, LayerNormalization,
    MultiHeadAttention, Add, Embedding, Reshape, Concatenate, Flatten, Activation
)
...
def create_hybrid_model(
    CNN_layers, filter1, filter2, filter3,
    kernel, dropout, pool_size,
    transformer_layers, patch_size,
    ...
    # CNN path
    x = Conv2D(filter1, (kernel, kernel), padding="same", activation="relu")(inputs)
    x = BatchNormalization()(x)
    x = MaxPooling2D(pool_size=(pool_size, pool_size), strides=1, padding="same")(x)
    x = Dropout(dropout)(x)

    if CNN_layers >= 2:
        x = Conv2D(filter2, (kernel, kernel), padding="same", activation="relu")(x)
        x = BatchNormalization()(x)
        x = MaxPooling2D(pool_size=(pool_size, pool_size), padding="same")(x)
        x = Dropout(dropout)(x)

    if CNN_layers >= 3:
        x = Conv2D(filter3, (kernel, kernel), padding="same", activation="relu")(x)
        x = BatchNormalization()(x)
    ...
    pos_embed = Embedding(input_dim=patch_dims, output_dim=projection_dim)(positions)
    ...
    transformer_output = LayerNormalization(epsilon=1e-6)(encoded)
    transformer_output = GlobalAveragePooling1D()(transformer_output)

    # Combine
    combined = Concatenate()([cnn_output, transformer_output])
    combined = BatchNormalization()(combined)
    combined = mlp(combined, hidden_units=mlp_head_units, dropout=dropout)
...
try:
    ...
    model = create_hybrid_model(**model_params)

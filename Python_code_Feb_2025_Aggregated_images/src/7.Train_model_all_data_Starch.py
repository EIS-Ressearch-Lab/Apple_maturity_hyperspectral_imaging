...
#     model_complex_firmness.add(BatchNormalization())
#     model_complex_firmness.add(MaxPooling2D(pool_size=(2, 2), strides=(1, 1), padding = 'same'))
#     model_complex_firmness.add(Dropout(0.25))
...
#     model_complex_firmness.add(Conv2D(2048, kernel_size=(2, 2), activation="relu")) #2048 - number of filters
#     model_complex_firmness.add(BatchNormalization())
#     model_complex_firmness.add(MaxPooling2D(pool_size=(2, 2), padding = 'same'))
#     model_complex_firmness.add(Dropout(0.25))
...
with strategy.scope():
    ...
    model_complex_starch_only.add(MaxPooling2D(pool_size=(2, 2), strides=(1,1), padding = 'same')) # cuts image in half
    model_complex_starch_only.add(Dropout(0.15))
    ...
    # Convolutional Block 4
    model_complex_starch_only.add(Conv2D(512, kernel_size=(4, 4), activation="relu"))
    model_complex_starch_only.add(BatchNormalization())
    model_complex_starch_only.add(MaxPooling2D(pool_size=(4, 4), padding = 'same'))
    model_complex_starch_only.add(Dropout(0.15))
...
# To load model architecture (run1)
# import tensorflow as tf
...
csv_logger = CSVLogger(f"{save_file_path}{Code_run_ID}_history_starch.csv", append=True)
...
model_complex_starch_only.fit(train_generator,epochs=110,steps_per_epoch=len(X_train_Starch) // batch_size,validation_data=val_generator,validation_steps=len(X_validate_Starch) // batch_size,callbacks=[tf.keras.callbacks.CSVLogger(f"{save_file_path}{Code_run_ID}history_starch.csv", append=True), checkpoint_starch_only],)


# In[ ]:


model_complex_starch_only.save(f'{save_file_path}{Code_run_ID}model_trained_2DCNN_starch.keras')

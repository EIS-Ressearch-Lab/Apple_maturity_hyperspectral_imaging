...
selected_features_threshold_brix = None
...
# model_path = f'{results_path}{run_id}_*model_trained_vit_brix.keras'  # 模型文件路径 # Model file path

# 查找最新的模型文件 # Find the latest model files
def find_latest_model(model_dir='/media/2tbdisk3/data/Haidee/Results', run_no=None):
    if run_no is not None:
        model_files = glob.glob(f"{model_dir}/*run_{run_no}*model_trained_vit_brix.keras")
    else:
        model_files = glob.glob(f"{model_dir}/*model_trained_vit_brix.keras")
    if not model_files:
...
pattern = f'{results_path}/*{run_no}*_history_brix_vit.csv'
print(pattern)
...
def predict_and_visualize(model, X_test, Y_test, test_encoder, img_size, save_dir):
    ...
    test_generator = data_generator_w_cultivar(X_test, Y_test, test_encoder, batch_size, img_size, band_filter=selected_features_threshold_brix)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_vit_test_predictions.png', dpi=300)
    plt.show()
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_vit_error_distribution.png', dpi=300)
    plt.show()
    ...
    results_df.to_csv(f'{save_dir}/brix_vit_prediction_results.csv', index=False)
...
def visualize_by_cultivar(results_df, test_encoder, save_dir):
    ...
    cultivar_metrics_df.to_csv(f'{save_dir}/brix_vit_cultivar_metrics.csv', index=False)
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_vit_cultivar_mae.png', dpi=300)
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_vit_cultivar_predictions.png', dpi=300)
    plt.show()
...
def visualize_training_history(history_path, save_dir):
    ...
    plt.tight_layout()
    plt.savefig(f'{save_dir}/brix_vit_training_history.png', dpi=300)
    plt.show()
